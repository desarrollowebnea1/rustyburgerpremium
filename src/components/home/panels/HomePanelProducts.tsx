...
function HorizontalProductCard({ product, index }: { product: HomePanelProduct; index: number }) {
  return (
    <motion.article
      data-carousel-card
    ...
    >
      <div className="relative aspect-[4/5] overflow-hidden border-2 border-rusty-gray bg-rusty-carbon shadow-[0_20px_40px_rgba(0,0,0,0.5)]">
        ...
        <div className="absolute inset-x-0 bottom-0 z-10 bg-gradient-to-t from-[#090909] via-[#090909]/85 to-transparent px-3 pb-3 pt-12">
          ...
          <p className="mt-0.5 font-display text-base text-rusty-orange md:text-lg">{product.priceLabel}</p>
          <AddToCartButton
...
function VerticalProductCard({ product }: { product: HomePanelProduct }) {
  return (
    <motion.article
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-4">
        <h3 className="font-display text-xl uppercase leading-tight tracking-wide text-rusty-cream">{product.name}</h3>
        <p className="mt-1 font-display text-lg text-rusty-orange">{product.priceLabel}</p>
        <AddToCartButton
          product={homeProductToCartInput(product)}
          label="Agregar"
          className="mt-4 w-full py-3 text-[11px] tracking-[0.14em]"
        />
      </div>
    </motion.article>
  );
...
function ProductsHorizontal({ products }: { products: HomePanelProduct[] }) {
        ...
        <div className="mt-6 flex items-center gap-3 md:mt-8 md:gap-4">
          ...
          >
            {loopedProducts.map((p, i) => (
              <HorizontalProductCard key={p.loopKey} product={p} index={i} />
            ))}
          </div>
...
function ProductsVertical({ products }: { products: HomePanelProduct[] }) {
  return (
...
export function HomePanelProducts({ layout = "vertical" }: { layout?: PanelLayout }) {
  ...
  return layout === "horizontal" ? (
    <ProductsHorizontal products={products} />
  ) : (
    <ProductsVertical products={products} />
  );
}
