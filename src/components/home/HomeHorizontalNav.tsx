...
const linkClass =
  "font-condensed text-[11px] font-semibold uppercase tracking-[0.12em] transition duration-500 hover:text-rusty-orange [text-shadow:0_0_12px_rgba(241,239,232,0.95),0_1px_2px_rgba(241,239,232,0.7)] md:text-[12px]";

/** Footer flotante — links internos en home, rutas en subpáginas */
export function HomeHorizontalNav() {
  const pathname = usePathname();
  const isHome = pathname === "/";
  const { activePanelId, scrollToPanel } = useHomeMotion();
        ...
        <ul className="flex flex-wrap items-center gap-x-5 gap-y-1 md:gap-x-8">
          {isHome
            ? HOME_NAV_LINKS.map((link) => {
                const isActive = activePanelId === link.panelId;
                return (
                  <li key={`${link.panelId}-${link.label}`}>
                    <button
                      type="button"
                      onClick={() => scrollToPanel(link.panelId)}
                      className={`${linkClass} ${
                        isActive ? "text-rusty-orange" : "text-[#0A0A0A]"
                      }`}
            ...
            : NAV_LINKS.map((link) => (
                <li key={link.href}>
                  <Link href={link.href} className={`${linkClass} text-[#0A0A0A]`}>
                    {link.label}
                  </Link>
                </li>
              ))}
